...
export function ListAllProfessionalBlocked({ listAllProfessional }: {listAllProfessional: ProfessionalProps[]}) {

  return (
    <main className="w-full">

      <header className="flex items-center justify-between mb-10 pt-4 pr-10">
        <h1 className="text-4xl text-zinc-500">Profissionais bloqueados</h1>
      ...
      <div className="w-full flex gap-5 animations">
        {listAllProfessional?.filter((user) => user.blocked).map((user) => (
          <Link href={`/all-professional/${user.id}`} key={user.cpf} className="w-full max-w-[300px] bg-gradient-to-r from-zinc-400 to-zinc-700 p-1 rounded-md cursor-pointer transition-all hover:scale-105">
